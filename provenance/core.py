...
#TODO: allow custom names and custom_fields to be passed in. Will require the provenance wrapper to inspect the function args.

@provenance(archive_file=True, name='archive_file', delete_original_file=True)
def _archive_file_delete_original(filename, custom_fields=None):
    return filename


@provenance(archive_file=True, name='archive_file', delete_original_file=False)
def _archive_file_preserve_original(filename, custom_fields=None):
    return filename

def archive_file(filename, delete_original=False, custom_fields=None):
    if delete_original:
        return _archive_file_delete_original(filename, custom_fields)
    else:
       return  _archive_file_preserve_original(filename, custom_fields)
