...
primitive_data = (
    st.floats(allow_nan=False) | st.booleans() | st.text() | st.none() | st.fractions() |
    st.integers() | st.characters()
)
...
builtin_data = st.recursive(
    primitive_data | sets,
    lambda children: st.lists(children) | st.dictionaries(st.text(), children) | st.
    tuples(children),
)
